...
const BusinessUserEdit:React.FC = () => {
    ...
    const { id } = useParams()
      ...
      entity_name: ""
        ...
        useEffect(() => {
      getRequest(`admin/business-users/${id}`)
        .then((res) => {
          ...
          user_details: [
            {
              ...
              postcode: customerDetails.pincode
                ...
                PutRequest(`admin/business-users/${Number(id)}`,payload)
                .then(()=>{
...
<div className='flex justify-between pb-8'>  <h1 className='flex items-center text-xl cursor-pointer' onClick={()=>navigate(-1)}> <ChevronLeft className='w-8 h-8' />{"Edit User"}</h1>
