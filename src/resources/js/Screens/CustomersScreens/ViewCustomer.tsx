    ...
    <Typography className='flex items-center w-max cursor-pointer' onClick={()=>navigate(-1)} sx={{...themes.mediumSizedFont,fontSize:25,color:"var(--color-purple)"}}>
            ...
            <div className=' flex flex-col gap-11'>
                <div className='grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-5 pt-6'>
                    ...
                    <RUDashBoardCard icon={Activity} count={`${!userDetails?.customer.pause_account?"active":"Inactive"}`} title='Status' />
                    <RUDashBoardCard icon={Scale} count={userDetails?.bookings.completed.length} title='Completed_Tasks' />
                    <RUDashBoardCard icon={Calendar} count={`${userDetails?.customer.email_verified_at?userDetails?.customer.email_verified_at.slice(0,10):"N/A"}`} title={`Member_Since`} />
                </div>
